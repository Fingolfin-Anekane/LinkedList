...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        try {
            auto it = values.end();
            std::advance(it, -1);
            while (std::distance(values.begin(), it) >= 0) {
                PushFront(*it);
                --it;
            }
    ...
    void PopFront() noexcept {
        auto second_el = begin().node_->next_node;
        delete before_begin().node_->next_node;
        before_begin().node_->next_node = second_el;
    }
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        ...
        pos.node_->next_node = next.node_;
        return Iterator(next.node_);
    }
